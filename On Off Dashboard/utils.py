...
class DataUtils:
    ...
    @staticmethod
    def apply_combined_filters(df, departments, account_names):
        """Applies both department and account name filters."""
        filtered_df = df[df['Department'].isin([d.lower() for d in departments]) &
                         df['account_name'].isin([a.lower() for a in account_names])]
        return filtered_df
